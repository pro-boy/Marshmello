...
USER_BOT_NO_WARN = ("`Hello, This is Marshmello SECURITY . ALERT TIME PASSERS FOUND your way here to my master,`"
                   f"{DEFAULTUSER}'s inbox. FUCK OFF SHIT DON'T PM\n"
                   f"\n**{CUSTOM_MIDDLE_PMP}**\n\n"
                    "`Leave your Name,Reason and 100000$ MILLION and hopefully you'll get a reply within 1000000 light years.`⭕️\n\n"
    ...
    async def approve_p_m(event):
        ...
        if event.is_private:
          if chat.id == 1289422521:
            await event.edit("You are tried to block my Creator😡 , Fuck Off now i am sleeping for 100 seconds")
            await asyncio.sleep(100)
          else:
            if pmpermit_sql.is_approved(chat.id):
                pmpermit_sql.disapprove(chat.id)
                await event.edit("Tu..😒 nikal pehli fursat me lawde ke baal gandu saale aa jate kaha kha se lawde log bosdike behenchod.!!**[{}](tg://user?id={})".format(firstname, chat.id))
                await asyncio.sleep(3)
...
async def hehehe(event):
    ...
    if event.is_private:
        if not pmpermit_sql.is_approved(chat.id):
            ...
            await borg.send_message(chat, "**Boss Meet My Creator he made Me The King pro @Beast_boy_shubu**")
